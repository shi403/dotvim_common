...
function! s:Initialize()
  ...
  let s:match_character = '\%([ウカキクケコサシスセソタチツテトハヒフヘホ]゛\|[ハヒフヘホ]゜\|.\)'
  let s:match_hankaku = '\%([ウカキクケコサシスセソタチツテトハヒフヘホ]゛\|[ハヒフヘホ]゜\|[ -~。「」、ヲァィゥェォャュョッーアイウエオカキクケコサシスセソタチツテトナニヌネノハヒフヘホマミムメモヤユヨラリルレロワン゛゜]\)'
  ...
  let han_kana = '。「」、ヲァィゥェォャュョッーアイウエオカキクケコサシスセソタチツテトナニヌネノハヒフヘホマミムメモヤユヨラリルレロワン゛゜'
  ...
  let tmp = tmp . '。。「「」」、、ヲヲァァィィゥゥェェォォャャュュョョッッ'
  let tmp = tmp . 'ーーアアイイウウエエオオカカキキククケケココササシシススセセソソ'
  let tmp = tmp . 'タタチチツツテテトトナナニニヌヌネネノノハハヒヒフフヘヘホホママ'
  let tmp = tmp . 'ミミムムメメモモヤヤユユヨヨララリリルルレレロロワワンン゛゛゜゜'
  let tmp = tmp . 'ウ゛ヴガガギギググゲゲゴゴザザジジズズゼゼゾゾ'
  let tmp = tmp . 'ダダヂヂヅヅデデドドババビビブブベベボボ'
  let tmp = tmp . 'パパピピププペペポポ'
  let tmp = tmp . ''
  let s:table_h2z = tmp
